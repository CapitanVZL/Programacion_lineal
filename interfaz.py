...
class InterfazProgramacionLineal:
    ...
    def configurar_estilos(self):
        ...
        self.style.configure("Signo.TMenubutton", padding=[2, 2, 2, 2], arrowsize=8, font=("Segoe UI", 10))
        self.style.configure("TSpinbox", arrowsize=10, background=ENTRY_BG, foreground=LIGHT_TEXT,
    ...
    def variables_interfaz(self):
        ...
        self.num_restricciones_var = tk.IntVar(value=2)
        self.tipo_optimizacion_var = tk.StringVar(value="Maximizar")
        self.metodo_resolucion_var = tk.StringVar(value="Dos Fases")

        self.crear_widgets_configuracion()
        self.crear_widgets_io()  # Solo crea, no muestra
    ...
    def crear_widgets_io(self):
        ...
        self.frame_restricciones = ttk.Frame(self.frame_io, padding="10 0 10 10")
        self.frame_resultados = ttk.Frame(self.root)
        self.boton_resolver = ttk.Button(self.frame_io, text="Resolver", style="Accent.TButton", command=self.calcular)
    ...
    def generar_campos_problema(self):
        ...
        self.frame_io.grid(row=2, column=0, padx=10, pady=10, sticky="nsew")
        ...
        self.boton_resolver.grid(row=2, column=0, padx=10, pady=20, sticky="ew")
    ...
    def _generar_campos_funcion_objetivo(self):
        ...
        for i in range(self.num_variables_var.get()):
            ...
            ttk.Spinbox(fo_entries_frame, textvariable=signo_var, values=("+", "-"), width=2, wrap=True,
    ...
    def _generar_campos_restricciones(self):
        ...
        for r_idx in range(self.num_restricciones_var.get()):
            ...
            for i in range(self.num_variables_var.get()):
                ...
                ttk.Spinbox(row_frame, textvariable=signo_var, values=("+", "-"), width=2, wrap=True,
                            state='readonly').pack(side="left", padx=4)
            ...
            self.restricciones_data.append(
                {"coeficientes": entries, "signos_vars": signos_vars, "signo_desigualdad": signo_desigualdad_var,
                 "rhs": rhs_entry})

    def calcular(self):
        datos_problema = self.obtener_datos_problema()
        if datos_problema:
            self.frame_resultados.grid(row=1, column=1, rowspan=2, padx=10, pady=10, sticky="nsew")
            resultado = self.calcular_callback(datos_problema)
            self.mostrar_resultados(resultado)

    def obtener_datos_problema(self):
        try:
            coef_fo = [float(e.get() or 0) * (1 if s.get() == '+' else -1) for e, s in
                       zip(self.entry_funcion_objetivo_vars, self.signos_fo_vars)]
            restricciones = [{"coeficientes": [float(e.get() or 0) * (1 if s.get() == '+' else -1) for e, s in
                                               zip(r["coeficientes"], r["signos_vars"])],
                              "signo": r["signo_desigualdad"].get(), "rhs": float(r["rhs"].get() or 0)} for r in
                             self.restricciones_data]
            return {"tipo_optimizacion": self.tipo_optimizacion_var.get(), "funcion_objetivo": coef_fo,
                    "restricciones": restricciones, "metodo": self.metodo_resolucion_var.get()}
        except ValueError:
    ...
    def mostrar_resultados(self, resultado):
        ...
        if resultado.get('metodo') in ['Simplex', 'Dos Fases']:
            self.mostrar_resultados_tabulares(resultado)
        else:
            self.mostrar_resultados_grafico(resultado)

    def mostrar_resultados_tabulares(self, resultado):
        ...
        titulo = f"=== ANÁLISIS MÉTODO {resultado.get('metodo', '').upper()} ==="
        text_resultados.insert(tk.END, f"{titulo}\n\n", "title");
        ...
        if not historial:
            ...
            for paso in historial:
                ...
                if paso.get('tableau') is not None:
                    tabla_str = self._formatear_tableau(paso['tableau'], paso.get('nombres_columnas', []))
    ...
    def _formatear_tableau(self, tableau, nombres_columnas):
        if not nombres_columnas: return "Error: Nombres de columna no proporcionados."
        base_vars = self._identificar_variables_base(tableau, nombres_columnas)
        headers = ["Base"] + nombres_columnas
        filas_str = ["".join(f"{h:<9}" for h in headers), "-" * len(headers) * 9]
        for i, fila in enumerate(tableau): filas_str.append(
            f"{base_vars[i]:<9}" + "".join(f"{val:<9.2f}" for val in fila))
        return "\n".join(filas_str)

    def _identificar_variables_base(self, tableau, nombres_columnas):
        base_vars = [""] * tableau.shape[0];
        base_vars[0] = "Z"
        var_map = {i: nombre for i, nombre in enumerate(nombres_columnas)}
        for col_idx in range(1, tableau.shape[1] - 1):
            columna = tableau[1:, col_idx]
            if abs(np.sum(columna) - 1.0) < 1e-9 and np.count_nonzero(np.abs(columna)) == 1:
                fila_idx = np.where(abs(columna - 1.0) < 1e-9)[0][0] + 1
